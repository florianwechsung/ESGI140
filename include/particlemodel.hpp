...
class Barrier {
   ...
   public:
    ...
    double operator()(double x) {
        ...
        double tightest_end =
            contraction_start + funnel_length + tightest_length;
        double contraction_end =
            contraction_start + 2 * funnel_length + tightest_length;
        double width;
        if (x > contraction_start && x < tightest_start) {
            width = (contracted_width - normal_width) / (funnel_length) *
                        (x - contraction_start) +
                    normal_width;
        } else if (x >= tightest_start && x <= tightest_end) {
            width = contracted_width;
        } else if (x > tightest_end && x < contraction_end) {
            width = (normal_width - contracted_width) / funnel_length *
                        (x - tightest_end) +
                    contracted_width;
        } else {
            width = normal_width;
...
class SpeedSettings {
   public:
    ...
    double rho_1;
    double rho_2;
    SpeedSettings(double _v_min, double _rho_1, double _rho_2)
        : v_min(_v_min), rho_1(_rho_1), rho_2(_rho_2) {}
};
...
std::tuple<std::vector<double>, std::vector<std::vector<double>>,
           std::vector<std::vector<double>>, std::vector<std::vector<double>>>
solve_particle_model(std::vector<double>&, std::vector<double>&,
                     std::function<double(double)>& width,
                     std::function<double(double)>& slope,
                     SpeedSettings& settings, double tmax = 3600.,
                     double dt = 1., double rho_start = 4.);
